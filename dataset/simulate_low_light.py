...
def prepare_dataset(input_dir, output_dir, val_ratio=0.1, test_ratio=0.2, resize_dim=(224, 224)):
    """
    Prepares a low-light dataset by splitting a directory of high-res images,
    applying a custom degradation function, and resizing to 224x224.
    """
    ...
    for split, filenames in splits.items():
        ...
        for fname in tqdm(filenames):
            ...
            # --- Resize both high and low images to 224x224 ---
            rgb_resized = cv2.resize(rgb_image, resize_dim, interpolation=cv2.INTER_AREA)
            degraded_resized = cv2.resize(degraded_img, resize_dim, interpolation=cv2.INTER_AREA)
            # Save GT (high) and degraded (low)
            Image.fromarray(rgb_resized).save(os.path.join(high_dir, fname))
            Image.fromarray(degraded_resized).save(os.path.join(low_dir, fname))
    print("\n✅ Dataset preparation complete. Images saved in 224x224, RGB format.")
if __name__ == "__main__":
    ...
    prepare_dataset(
        ...
        val_ratio=0.1,
        test_ratio=0.2,
        resize_dim=(224, 224)   # final resize step
    )
